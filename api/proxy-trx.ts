...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: "Upstream TRX API error", details: errorText });
    }

    const data = await response.json();
    return res.status(200).json(data);
  } catch (error: any) {
    return res.status(500).json({ error: "Failed to fetch from TRX API", message: error.message });
  }
}
